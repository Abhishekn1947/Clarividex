  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-5 animate-fade-in">
      ...
      <div className="glass-card overflow-hidden">
        {/* Header — gradient bg */}
        <div className="bg-gradient-to-r from-slate-800 via-slate-900 to-slate-800 p-4 sm:p-6 text-white rounded-t-2xl">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <p className="text-sm text-slate-400 mb-1.5">Prediction</p>
              <h2 className="text-xl font-semibold leading-snug">{prediction.query}</h2>
              {prediction.ticker && (
                <div className="flex flex-wrap items-center gap-3 mt-3 text-sm text-slate-300">
                  <span className="font-mono bg-white/10 backdrop-blur-sm px-3 py-1 rounded-xl text-sm font-semibold">
                    {prediction.ticker}
                  </span>
            ...
            <div className="flex flex-col items-end gap-2">
              ...
              <div className={cn(
                "flex items-center gap-1.5 px-3 py-1 rounded-xl text-sm font-medium",
        ...
        <div className="p-5 sm:p-8">
          <div className="flex flex-col md:flex-row items-center justify-center gap-5 sm:gap-8">
            ...
            {/* Key Metrics Grid */}
            <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-xs">
              <div className="metric-card">
                <div className="text-sm text-slate-500 mb-1.5 flex items-center gap-1">
                  Confidence
                  <InfoTooltip term="confidence" />
                </div>
                <div className={cn("text-base font-semibold", confidence.color)}>
                  {confidence.label}
                </div>
              </div>
              ...
              <div className="metric-card">
                ...
                <div className={cn("text-base font-semibold flex items-center gap-1", sentiment.color)}>
      ...
      {/* AI Summary — amber accent border */}
          ...
          <div className="space-y-2.5">
            ...
            {prediction.reasoning.bullish_factors.length > 3 && (
              <button
                ...
                className="w-full text-sm text-slate-500 hover:text-slate-700 flex items-center justify-center gap-1 py-2"
              >
        ...
        <div className="card">
          ...
          <div className="space-y-2.5">
            ...
            {prediction.reasoning.bearish_factors.length === 0 && (
              <div className="text-slate-400 text-sm italic p-4 bg-slate-50 rounded-xl">
        ...
        <div className="card">
          ...
          <div className="space-y-2.5">
            {prediction.reasoning.catalysts.map((catalyst, index) => (
              <div key={index} className="flex items-center justify-between p-4 bg-slate-50/80 rounded-2xl backdrop-blur-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-slate-700 text-sm">{catalyst.event}</p>
                    {catalyst.date && (
                ...
                <span className={cn(
                  "px-2.5 py-1 rounded-xl text-xs font-medium capitalize",
          ...
          <ul className="space-y-2.5">
            {prediction.reasoning.risks.map((risk, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-slate-600">
                <span className="w-5 h-5 rounded-lg bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-medium flex-shrink-0 mt-0.5">
                  {index + 1}
                </span>
                {risk}
              </li>
        ...
        <div className="card">
          ...
          <ul className="space-y-2.5">
            {prediction.reasoning.assumptions.map((assumption, index) => (
              <li key={index} className="flex items-start gap-2 text-sm text-slate-600">
                <span className="w-5 h-5 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center text-xs font-medium flex-shrink-0 mt-0.5">
                  {index + 1}
                </span>
                {assumption}
              </li>
        ...
        <div className="card">
          ...
          <div className="space-y-2.5">
            {prediction.news_articles
              ...
              .map((article, index) => (
                <div key={index} className="flex items-start gap-3 p-3 rounded-xl bg-slate-50/80 hover:bg-slate-100/80 transition-colors backdrop-blur-sm">
                  <div className={cn(
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-700 line-clamp-2">{article.title}</p>
                  ...
                  {article.url && (
                    <a
                      ...
                      className="w-7 h-7 rounded-xl bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors flex-shrink-0"
                    >
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
            ...
            {prediction.technicals.rsi_14 !== null && (
              <div className="text-center p-4 rounded-2xl bg-slate-50/80 border border-slate-100 backdrop-blur-sm hover:shadow-md transition-shadow">
                <div className="text-sm text-slate-500 mb-1.5 flex items-center justify-center gap-1">
                  RSI (14)
                ...
                <div className={cn(
                  "text-xs mt-1.5 px-2 py-0.5 rounded-full inline-block",
            ...
            {prediction.technicals.support_level !== null && (
              <div className="text-center p-4 rounded-2xl bg-emerald-50/80 border border-emerald-100 backdrop-blur-sm hover:shadow-md transition-shadow">
                <div className="text-sm text-emerald-600 mb-1.5 flex items-center justify-center gap-1">
                  Support Level
            ...
            {prediction.technicals.resistance_level !== null && (
              <div className="text-center p-4 rounded-2xl bg-red-50/80 border border-red-100 backdrop-blur-sm hover:shadow-md transition-shadow">
                <div className="text-sm text-red-600 mb-1.5 flex items-center justify-center gap-1">
                  Resistance Level
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-medium text-amber-800 text-sm mb-1">Disclaimer</p>
            <p className="text-sm text-amber-700">{prediction.disclaimer}</p>
          </div>
        </div>
      </div>
